...
class TestProcessingLogic(unittest.TestCase):
    ...
    def test_fetch_and_process_pull_requests(
        self,
        mock_fetch_reviews,
        ...
        mock_fetch_reviews.return_value = [
            {
                ...
                "state": "APPROVED",
                "submitted_at": "2025-05-02T12:00:00Z",
            },
            {
                "user": {"login": "reviewer1"},
                "state": "COMMENTED",
    ...
    @patch("gh_pulls_summary.main.fetch_user_details")
    @patch("gh_pulls_summary.main.fetch_reviews")
    def test_author_name_fallback_to_username(
        self,
        mock_fetch_reviews,
        mock_fetch_user_details,
        mock_fetch_issue_events,
    ...
    @patch("gh_pulls_summary.main.fetch_user_details")
    @patch("gh_pulls_summary.main.fetch_reviews")
    def test_pr_body_url_text_extraction(
        self,
        mock_fetch_reviews,
        mock_fetch_user_details,
        mock_fetch_issue_events,
...
class TestGenerateMarkdownOutput(unittest.TestCase):
    ...
    def test_generate_markdown_output_sort_by_title(self, mock_fetch):
        ...
        expected_output = expected_output.replace("2025-02", "2025-05-02").replace(
            "2025-01", "2025-05-01"
        )
        self.assertEqual(markdown_output, expected_output)

    @patch("gh_pulls_summary.main.fetch_and_process_pull_requests")
    def test_generate_timestamp_with_generator(
        self, mock_fetch_and_process_pull_requests
    ):
        ...
        ts = generate_timestamp(
            ...
            generator_name="octocat",
            generator_url="https://github.com/octocat",
        )
        self.assertEqual(
            ts,
            "**Generated at 2025-07-02 12:00Z** by [octocat](https://github.com/octocat)\n",
        )
        # With name only
