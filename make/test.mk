...
test: install-package ## Run all tests (unit + integration)
	@RUN_INTEGRATION_TESTS=1 $(VENV_PYTHON) -m pytest tests/ -v $(ARGS)
...
test-integration: install-package ## Run integration tests only
	@RUN_INTEGRATION_TESTS=1 $(VENV_PYTHON) -m pytest tests/integration/ -v $(ARGS)
...
coverage-report: install-package ## Run tests with coverage report (no threshold check)
	@$(VENV_PYTHON) -m pytest tests/unit/ --cov=src --cov-report=html --cov-report=term --cov-report=xml $(ARGS)
	@$(VENV_PYTHON) -m coverage report | tail -1 | awk '{print $$4}' | sed 's/%//' > .coverage-percentage
	@printf "$(GREEN)✅ Coverage report generated (unit tests only)$(RESET)\n"
